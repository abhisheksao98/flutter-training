...
class englishdisplay extends StatefulWidget {
  const englishdisplay({Key? key}) : super(key: key);

  @override
  State<englishdisplay> createState() => _englishdisplayState();
}

class _englishdisplayState extends State<englishdisplay> {
  var eng = EnglishDbconnect();
  late Future _questions;
  Future<List<Question>> getData() async {
    return eng.englishfetchQuestions();
  }

  ResultController result = Get.put(ResultController());
  IndexController totalquestion = Get.put(IndexController());
  @override
  void initState() {
    _questions = getData();
    super.initState();
    ...
    return FutureBuilder(
        ...
        builder: (ctx, shot) {
          if (shot.connectionState == ConnectionState.done) {
            if (shot.hasError) {
              return Center(
                child: Text('${shot.error}'),
              );
            } else if (shot.hasData) {
              var extractedData = shot.data as List<Question>;
